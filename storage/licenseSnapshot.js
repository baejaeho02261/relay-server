...
let lastFile='',lastText='';
function SaveLicenseSnapshot(){const file=config.LICENSE_SNAPSHOT_FILE,bak=config.LICENSE_SNAPSHOT_BAK_FILE,tmp=file+'.tmp';try{const text=JSON.stringify(ObjectData(),null,2);if(file===lastFile&&text===lastText&&fs.existsSync(file)){require('../services/member/entryPass').Migrate();return true;}if(fs.existsSync(file))try{fs.copyFileSync(file,bak)}catch(_){}fs.writeFileSync(tmp,text,'utf8');fs.renameSync(tmp,file);lastFile=file;lastText=text;require('../services/member/entryPass').Migrate();return true;}catch(e){try{if(fs.existsSync(tmp))fs.unlinkSync(tmp)}catch(_){}console.error('LICENSE SNAPSHOT SAVE ERROR:',e.message);return false;}}
