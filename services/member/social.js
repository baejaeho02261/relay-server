...
function PublicNews(row,summary=false){
 ...
 if(summary){const text=[...String(row.body||'').replace(/\s+/gu,' ').trim()];result.summary=text.slice(0,140).join('');result.summaryTruncated=text.length>140;}
 return result;
}
...
function FeedChannel(value){
 if(value!==undefined&&typeof value!=='string')s.Fail('INPUT_INVALID');
 const channel=value===undefined?'':value;
 if(!['','PUBG','VALORANT'].includes(channel))s.Fail('INPUT_INVALID');
 return channel;
}
...
function FeedRows(p,body={}){
 ...
 const rows=Object.values(s.DB().posts).filter(x=>(!channel||x.channel===channel)&&!x.archived&&extra.Visible(x,p)&&require('./activity-settings').FeedVisible(p,x)&&(!body.mine||x.accountId===p.id)&&(!body.following||require('./follows').IsFollowing(p.id,x.accountId)));
...
function Feed(p,body){
 const page=s.Page(FeedRows(p,body),body,8);return {...page,channel:FeedChannel(body.channel),items:page.items.map(x=>PublicPost(x,p,false,body._wire==='zlib'))};
}
...
function ThreadRows(p,post){
 const visible=require('./readScope').By('comments','postId',post.id).filter(x=>!x.deleted&&!x.hidden&&!extra.Blocked(p.id,x.accountId)&&!s.ProfileById(x.accountId)?.blocked&&require('./activity-settings').CommentVisible(p,x));
...
module.exports={FeedChannel,NewsRows,PublicNews,UnreadNewsCount,FeedRows,ThreadRows,AvatarThumb,Article,EditPost,Avatar,SaveProfile,News,SaveNews,Feed,Popular,Thread,Post,Comment,Remove,React,Report,PublicPost,Author};
