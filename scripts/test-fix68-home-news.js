...
try{
 ...
 for(const key of ['news','catalog','orders','payments','pointHistory','purchases','topGames','recentProducts','recentServices'])assert.ok(result[key].length<=3,key+' bounded');assert.ok(result.feed.length<=10);assert.deepEqual(result.feed.map(row=>row.id),require('../services/member/social').FeedRows(a,{sort:'popular'}).slice(0,10).map(row=>row.id),'home uses the canonical viewer-specific Top10');assert.deepEqual(result.feed.map(row=>row.rank),result.feed.map((_,index)=>index+1),'ranks remain contiguous');
 ...
 const icon=news.slice(news.indexOf('procedure HubContentIcon'),news.indexOf('function HubContentCategoryHeading'));assert.ok(!icon.includes('UiRect'),'category icons have no plate');assert.ok(news.includes('Y+(LineH-Size)/2,Size'),'icon matches title size and aligns with the first title line');
 for(const route of ['baccarat','roulette','slots','blackjack','crash','dice','mines','plinko','limbo','hilo','tower','event.dino','event.flappy','event.whack','event.dodge','event.rhythm'])assert.ok(dashboard.includes("'"+route+"'"),'home route '+route);assert.ok(!dashboard.includes('MenuRows:=HubActivityMenuRows'),'removed settings groups are not reinserted into Home');
 for(const title of ['내 지갑','대화와 알림','돌림판 참여 횟수','내 MoaPlay 사용 방식','구독','내 콘텐츠를 볼 수 있는 사람','다른 사람이 나와 소통할 수 있는 방법','내가 볼 수 있는 내용','내 앱 및 미디어','가족 센터','내 인사이트 및 도구','내 주문 및 거부 캠페인','더 많은 정보 및 지원','Meta의 다른 앱','설정 및 활동','나의 활동'])assert.ok(!dashboard.includes("StartCard('"+title+"'"),'removed Home group '+title);
 assert.ok(dashboard.includes("StartCard('내 계정','accounts','activity.account')"),'Account Center remains available');
 assert.ok(dashboard.includes("StartCard('피드 Top 10','feed','popular')"),'Home exposes the Top10 collection');assert.ok(!dashboard.includes("Tiles(['작성하기','인기 피드']"),'retired feed buttons stay removed');
