...
try{
 ...
 assert.equal(data.payments[0].reference,'ORDER-3');assert.equal(data.payments[0].paymentMethod,'WALLET');assert.equal(data.events.rules.revision,rewards.Rules().revision);assert.deepEqual(data.events.rules.prizes,rewards.Rules().prizes);
 ...
 assert.ok(ui.includes("StartCard('MY'"));assert.ok(ui.includes("'points.exchange.open'"));assert.ok(ui.includes("'activity.subscriptions'"));
 assert.ok(ui.indexOf("StartCard('출석 랭킹'")<ui.indexOf("StartCard('돌림판'"));assert.ok(ui.indexOf("StartCard('돌림판'")<ui.indexOf("StartCard('새로운 소식'"));
 assert.ok(ui.includes("HubFillWheel(Card,HubObject(Events,'rules'),Events,Y+2,218)"));assert.ok(wheel.includes('function TMoaPlayForm.HubFillWheel'));assert.ok(wheel.includes("Hub.TagString:='event.spin'"));
 ...
 const pointsBefore=a.points,spinsBefore=a.eventSpins,request='FIX71-SPIN-RETRY';
 const spin=service.Execute(ca,request,'event.spin',{revision:data.events.rules.revision});
 const pointsAfterFirst=a.points,ledgerAfterFirst=Object.values(s.DB().pointLedger).filter(row=>row.accountId===a.id&&row.kind==='ROULETTE');
 const repeated=service.Execute(ca,request,'event.spin',{revision:data.events.rules.revision});
 assert.equal(spin.spin.id,repeated.spin.id);assert.equal(a.eventSpins,spinsBefore-1);assert.equal(a.points,pointsAfterFirst,'retry cannot credit either the wheel or an earned badge twice');assert.ok(pointsAfterFirst>=pointsBefore+spin.spin.points);assert.equal(ledgerAfterFirst.length,1);assert.equal(ledgerAfterFirst[0].amount,spin.spin.points);assert.equal(Object.values(s.DB().pointLedger).filter(row=>row.accountId===a.id&&row.kind==='ROULETTE').length,1);
 const afterSpin=home.Read(a);assert.equal(afterSpin.events.spins,spinsBefore-1);assert.equal(afterSpin.wallet.points,a.points);
 console.log('FIX71 HOME PASS: five full canonical popular posts, private/hidden content filtered, five recent comments, honest streak ranking/presence/KST daily counts, two icon games, bounded owner receipts, no read writes or catalog media, relative clocks and shared inline renderers.');
